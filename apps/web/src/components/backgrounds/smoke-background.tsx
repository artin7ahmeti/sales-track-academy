...
class Renderer {
  ...
  render(now = 0) {
    ...
    gl.uniform2f((program as never as Record<string, WebGLUniformLocation>).resolution, canvas.width, canvas.height);
    gl.uniform1f((program as never as Record<string, WebGLUniformLocation>).time, now * 1e-3);
    gl.uniform3fv((program as never as Record<string, WebGLUniformLocation>).u_color, this.color);
...
export function SmokeBackground({ smokeColor = '#1a1a3e', className }: SmokeBackgroundProps) {
